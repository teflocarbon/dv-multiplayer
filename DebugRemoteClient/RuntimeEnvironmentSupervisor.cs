...
internal sealed class RuntimeEnvironmentSupervisor : IDisposable
{
    ...
    private async Task Run(RuntimeEnvironmentStartRequestDto request, CancellationToken token)
    {
        try
        {
            ...
            Set(RuntimeEnvironmentStage.LoadingHostSave, "Loading the host's latest save through Derail Valley's Continue action.");
            await Command(hostSession, "environment.host-latest-save", RuntimeTestMutationKind.IsolatedMutation,
    ...
    private static string JoinArguments(string first, string second) => string.Join(" ", new[] { first, second }.Where(value => !string.IsNullOrWhiteSpace(value)));
    private static void Validate(RuntimeEnvironmentStartRequestDto request)
    {
    ...
    private void Set(RuntimeEnvironmentStage stage, string message) { lock (gate) { status.Stage = stage; status.Message = message; status.Error = string.Empty; status.Active = stage is not RuntimeEnvironmentStage.Idle and not RuntimeEnvironmentStage.Stopped and not RuntimeEnvironmentStage.Failed; status.Ready = stage == RuntimeEnvironmentStage.Ready; status.UpdatedUtc = DateTime.UtcNow; } }
    ...
    private static RuntimeEnvironmentStatusDto NewStatus(RuntimeEnvironmentStage stage, string message) => new() { Stage = stage, Message = message, Active = stage is not RuntimeEnvironmentStage.Idle and not RuntimeEnvironmentStage.Stopped and not RuntimeEnvironmentStage.Failed, Ready = stage == RuntimeEnvironmentStage.Ready, UpdatedUtc = DateTime.UtcNow };
    private static RuntimeEnvironmentStatusDto Clone(RuntimeEnvironmentStatusDto value) => new() { Stage = value.Stage, Active = value.Active, Ready = value.Ready, Message = value.Message, Error = value.Error, UpdatedUtc = value.UpdatedUtc, HostProcessId = value.HostProcessId, ClientProcessId = value.ClientProcessId, HostSessionId = value.HostSessionId, ClientSessionId = value.ClientSessionId, HostReadiness = new(value.HostReadiness), ClientReadiness = new(value.ClientReadiness) };
    public void Dispose()
    {
        lifetime.Cancel();
        operation?.Cancel();
