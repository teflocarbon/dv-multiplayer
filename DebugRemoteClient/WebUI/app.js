...
function renderReplicationList() {
  ...
  for (const operation of visible) {
    ...
    const route = document.createElement("span"); route.className = "route"; route.textContent = `${sessionLabel(operation.originSessionId)} · ${operation.stageCount} stages · ${operation.appliedRecipientCount}/${operation.recipientCount} applied${operation.discontinuityReason ? ` · ${operation.discontinuityReason}` : ""}`;
